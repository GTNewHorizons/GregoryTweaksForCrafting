...
public class ClientProvider {
    ...
    public void balanceGrid(TweakProvider provider, EntityPlayer entityPlayer, Container container, int id) {
        ...
        ItemStackMap<List<Slot>> balanceSlots = new ItemStackMap<>(true);
        ...
        for (int i = start; i < start + size; i++) {
            ...
            if (slot.getHasStack()) {
                ...
                balanceSlots.computeIfAbsent(itemStack, key -> new ArrayList<>())
                    .add(slot);
            }
        }
        for (Map.Entry<ItemStack, List<Slot>> entry : balanceSlots.entrySet()) {
            Collection<Slot> slotList = entry.getValue();
            int average = 0;

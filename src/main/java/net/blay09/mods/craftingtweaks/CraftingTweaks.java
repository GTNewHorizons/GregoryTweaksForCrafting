...
import com.gtnewhorizons.craftingtweaks.Tags;
...
@Mod(
    ...
    dependencies = "required-after:gtnhlib@[0.9.24,);",
...
public class CraftingTweaks {
    ...
    private static Configuration addonConfig;
    ...
    public void preInit(FMLPreInitializationEvent event) {
        ...
        try {
            ConfigurationManager.registerConfig(CraftingTweaksConfig.class);
        } catch (ConfigException e) {
            throw new RuntimeException("Failed to register Crafting Tweaks config.", e);
        }
        ...
        addonConfig = new Configuration(event.getSuggestedConfigurationFile());
        migrateGeneralSettingsIfNeeded(event, addonConfig);
        addonConfig.setCategoryComment(
            "addons",
        ...
        for (Property property : addonConfig.getCategory("addons")
            .values()) {
            configMap.put(
                property.getName(),
    ...
    public void postInit(FMLPostInitializationEvent event) {
        ...
        Compatibility.appliedenergistics2();
        Compatibility.avaritia();
        Compatibility.backpacks();
        Compatibility.bibliocraft();
        Compatibility.bluepower();
        Compatibility.buildcraft();
        Compatibility.jacb();
        Compatibility.ganyssurface();
        Compatibility.forestry();
        Compatibility.draconicevolution();
        Compatibility.natura();
        Compatibility.minefactoryreloaded();
        Compatibility.twilightforest();
        Compatibility.tinkersconstruct();
        Compatibility.thermalexpansion();
        Compatibility.railcraft();
        Compatibility.thaumcraft4();
        Compatibility.rotarycraft();
        Compatibility.terrafirmacraft();
    ...
    public static void saveConfig() {
        ConfigurationManager.save(CraftingTweaksConfig.class);
    }
    ...
    private static void migrateGeneralSettingsIfNeeded(FMLPreInitializationEvent event, Configuration legacyConfig) {
        File newConfigFile = new File(event.getModConfigurationDirectory(), "craftingtweaks-general.cfg");
        if (newConfigFile.exists()) {
            return;
        }
        CraftingTweaksConfig.hideButtons = legacyConfig.getBoolean(
            "hideButtons",
            "general",
            false,
            "This option is toggled by the 'Toggle Buttons' key that can be defined in the Controls settings.");
        CraftingTweaksConfig.hideButtonTooltips = legacyConfig.getBoolean(
            "hideButtonTooltips",
            "general",
            false,
            "Set this to true if you don't want the tweak buttons' tooltips to show.");
        CraftingTweaksConfig.compressAnywhere = legacyConfig.getBoolean(
            "compressAnywhere",
            "general",
            false,

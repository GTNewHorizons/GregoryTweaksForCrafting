...
public class DefaultProviderImpl implements DefaultProvider {
    ...
    public void balanceGrid(EntityPlayer entityPlayer, Container container, IInventory craftMatrix, int start,
        int size) {
        ItemStackMap<List<ItemStack>> itemMap = new ItemStackMap<>(true);
        ItemStackMap<Integer> itemCount = new ItemStackMap<>(true);
        for (int i = start; i < start + size; i++) {
            ItemStack itemStack = craftMatrix.getStackInSlot(i);
            if (itemStack != null && itemStack.getMaxStackSize() > 1) {
                itemMap.computeIfAbsent(itemStack, key -> new ArrayList<>())
                    .add(itemStack);
                itemCount.merge(itemStack, itemStack.stackSize, Integer::sum);
            }
        }
        for (Map.Entry<ItemStack, List<ItemStack>> entry : itemMap.entrySet()) {
            List<ItemStack> balanceList = entry.getValue();
            Integer totalCount = itemCount.get(entry.getKey());
            if (totalCount == null || balanceList.isEmpty()) {
                continue;
            }
